            ...
            <AnimatePresence>
                {error && (
                    <motion.div
                        ...
                        className="fixed top-6 left-1/2 z-[200] bg-black/90 border-2 border-pink-500 rounded-2xl px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 shadow-[0_0_30px_rgba(255,0,255,0.4)] w-[90%] max-w-lg"
                    >
                        <div className="flex items-start gap-3 w-full sm:flex-1">
                            <AlertCircle className={`w-6 h-6 shrink-0 mt-0.5 animate-pulse ${error === 'SAFETY_VIOLATION' ? 'text-red-500' : 'text-pink-500'}`} />
                        ...
                        <button 
                            ...
                            className={`w-full sm:w-auto px-4 py-2 sm:px-3 sm:py-1.5 rounded-lg border transition-colors text-xs font-bold uppercase tracking-wider shrink-0 text-center
                                ${error === 'SAFETY_VIOLATION' 
